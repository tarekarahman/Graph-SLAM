...
class robot:
    ...
    def __repr__(self):
        return 'Robot FInal Pose: [x=%.5f y=%.5f]'  % (self.x, self.y)
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        list2=[0,1] +list( range(4,dim+2))
        
        #print((dim+2),len(list2))
        Omega=Omega.expand(dim+2,dim+2,list2,list2)
        Xi=Xi.expand(dim+2,1,list2,[0])
        # update the information maxtrix/vector based on the robot motion
        for b in range(4):
...
for i in range(len(TimeSteps)):
    ...
    ticFullSlam = time.time()
    
    result = slam(data, N, num_landmarks, motion_noise, measurement_noise)
    
    TFullSLAM[i]=time.time()-ticFullSlam

    print("Full Graph SLAM-retains entire path and all landmarks")
    print_result(N, num_landmarks, result)
    print()
#    print("Full SLAM Processing Time")
#    print(TFullSLAM[i])
#    print()
    ...
    TOnline[i]=time.time()-ticOnline

    print()
    print("Online SLAM- retains all landmarks but only most recent robot pose")
    print_result(1, num_landmarks, result[0])
    print()
#    print("Online SLAM Processing Time")
#    print(TOnline[i])

#
#plt.figure()
#ax=plt.subplot()
#ax.plot(TimeSteps, TFullSLAM,label='Full SLAM ')
#ax.plot(TimeSteps, TOnline,label='Online SLAM')
#plt.xlabel('Number of Time Samples')
#plt.ylabel('Processing Time (sec)')
#ax.legend()
#plt.title('Graph SLAM with 40 Landmarks')
#plt.draw()
